...
class LoginViewModel @Inject constructor(
  ...
  fun login() {
    ...
    viewModelScope.launch {
      val result = authService.login(_loginUiState.value.email, _loginUiState.value.password)
      _loginUiState.value = _loginUiState.value.copy(
        loginState = if (result.isSuccess) AuthUiState.Success else AuthUiState.Failure(result.exceptionOrNull())
      )
    }
  }
  ...
  fun sendPasswordResetEmail() {
    ...
    viewModelScope.launch{
      try {
        authService.sendResetPasswordEmail(_loginUiState.value.forgetPasswordEmail)
        _loginUiState.value = _loginUiState.value.copy(loginState = AuthUiState.Idle)
      } catch (exception: Exception) {
        _loginUiState.value = _loginUiState.value.copy(loginState = AuthUiState.Failure(exception))
      }
    }
  }

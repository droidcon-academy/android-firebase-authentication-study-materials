...
fun PhoneAuthScreen(
    ...
    object : PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
      override fun onVerificationCompleted(credential: PhoneAuthCredential) {
        Toast.makeText(context,
          context.getString(R.string.verification_successful), Toast.LENGTH_SHORT).show()
        loading.value = false
      }

      override fun onVerificationFailed(exception: FirebaseException) {
        Toast.makeText(context,
          context.getString(R.string.verification_failed, exception.message), Toast.LENGTH_LONG).show()
        loading.value = false
      }
  ...
  Box(modifier = Modifier.fillMaxSize()) {
    Column(
      ...
      Text(
        text = stringResource(R.string.use_phone_authentication),
        fontSize = 24.sp,
      ...
      Text(
        text = stringResource(R.string.please_provide_your_phone_number),
        fontSize = 16.sp,
        fontWeight = FontWeight.Medium,
        ...
        Button(
          ...
          onClick = {
            if (phoneNumber.isPhoneNumberNotValid()) {
              Toast.makeText(context, R.string.please_enter_a_valid_phone_number, Toast.LENGTH_SHORT).show()
            } else {
              loading.value = true
              val options = PhoneAuthOptions.newBuilder(firebaseAuth)
                .setPhoneNumber("+44$phoneNumber")
      ...
      AnimatedVisibility(visible = codeSent.value) {
        Column {
          ...
          Button(
            ...
            enabled = !loading.value,
            onClick = {
              if (otp.isNotOtpValid()) {
                Toast.makeText(context, R.string.please_enter_a_correct_otp, Toast.LENGTH_SHORT).show()
              } else {
                loading.value = true
                coroutineScope.launch {
                   val credentials = PhoneAuthProvider.getCredential(verificationID.value, otp)
                  try {
                    val result = firebaseAuth.signInWithCredential(credentials).await()
                    if (result.user != null) onPhoneAuthSuccess()
                  } catch (e: Exception) {
                    loading.value = false
                    if (e is FirebaseAuthInvalidCredentialsException){
                      Toast.makeText(context,
                        context.getString(R.string.sorry_verification_failed_due_to) + e.message,
                        Toast.LENGTH_LONG)
                        .show()
...
private fun String.isNotOtpValid() = this.isEmpty() || this.length < 6

...
fun AuthScreen(
  ...
  onStartMultiFactor: (MultiFactorResolver) -> Unit = {},
) {
  Column(
          ...
          if (it.resultCode == Activity.RESULT_OK) {
            ...
            if (credentials.googleIdToken != null) {
              ...
              coroutineScope.launch {
                ...
                } catch (exception: FirebaseAuthMultiFactorException) {
                  val resolver = exception.resolver
                  onStartMultiFactor(resolver)
                }
              }
            }
          }
    ...
    val facebookLauncher =
      rememberLauncherForActivityResult(contract = loginManager.createLogInActivityResultContract(
        callbackManager,
        null
      ),
        onResult = {})
...
fun createSignInRequest(): BeginSignInRequest {
  return BeginSignInRequest.builder().setGoogleIdTokenRequestOptions(
    BeginSignInRequest.GoogleIdTokenRequestOptions.builder().setSupported(true)
      .setServerClientId("693836462692-3ukv3j1tg716juoct8n9hprs98k497fs.apps.googleusercontent.com")
      .build()
  ).build()
}

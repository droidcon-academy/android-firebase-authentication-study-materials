...
fun MultiFactorPhoneScreen(
  onBackClicked: () -> Unit = {},
  onAddingMultiFactorSuccess: () -> Unit = {},
  firebaseAuth: FirebaseAuth,
  session: MultiFactorSession?,
  hint: PhoneMultiFactorInfo?,
  onVerifyMultiFactor : (MultiFactorAssertion) -> Unit
) {
  ...
  Box(modifier = Modifier.fillMaxSize()) {
    Column(
      ...
      val titleHeader = if (hint == null) "Add MultiFactor Auth" else "Verify Second Factor"
      Text(
        text = titleHeader,
        fontSize = 24.sp,
      ...
      val subTitle = if (hint == null) "Please Provide your phone number for Multi Factor" else "We're verifying for ${hint.phoneNumber}"
      Text(
        text = subTitle,
        fontSize = 16.sp,
        fontWeight = FontWeight.Medium,
      ...
      AnimatedVisibility(
        ...
        Button(
          ...
          onClick = {
            if (hint == null && phoneNumber.isPhoneNumberNotValid()) {
              Toast.makeText(
              ...
              coroutineScope.launch {
                firebaseAuth.sendPhoneVerificationSMS(
                  ...
                  context as Activity,
                  "+44$phoneNumber",
                  session = session?: firebaseAuth.currentUser?.multiFactor?.session?.await(),
                  hint
                )
              }
            }
          ...
          Button(
            ...
            onClick = {
              if (otp.isOtpNotValid()) {
                ...
                if (session != null){
                  onVerifyMultiFactor(multiFactorAssertion)
                } else {
                  firebaseAuth.currentUser?.multiFactor?.enroll(multiFactorAssertion, "Phone Number")
                    ?.addOnSuccessListener { onAddingMultiFactorSuccess() }
...
private fun FirebaseAuth.sendPhoneVerificationSMS(
  ...
  if (session!=null) options.setMultiFactorSession(session)
  if (hint != null) options.setMultiFactorHint(hint) else options.setPhoneNumber(phoneNumber)
